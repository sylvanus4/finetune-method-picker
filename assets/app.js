...
(function () {
  ...
  const t = k => I18N.t(k);                          // chrome string by key
  const L = v => I18N.L(v);                          // data-prose {ko,en} by lang (plain string passes through)
  const ti = (k, p) => { let s = I18N.t(k); for (const key in p) s = s.replace("{" + key + "}", p[key]); return s; };
  ...
  function applyPresetObj(ps, announce, origin) {
    ...
    if (announce) renderPresetNote(ps, origin);
  }

  let lastPs = null, lastOrigin = null;
  function renderPresetNote(ps, origin) {
    lastPs = ps; lastOrigin = origin;
    const gName = (GPUS.find(g => g.id === ps.gpu) || {}).name || ps.gpu;
    const tail = origin === "gpu" ? t("recTailGpu") : t("recTailModel");
    $("presetNote").style.display = "";
    $("presetNote").innerHTML = `${t("recSetup")}: <b>${ps.tuning.toUpperCase()}</b> · <b>${gName.split(" (")[0]} ×${ps.numGpus}</b> · LR ${ps.lr} · ${ps.optimizer} · batch ${ps.perDeviceBatch}×accum ${ps.gradAccum} · seq ${ps.seqLen} · ${ps.epochs}ep <span class="dim">${tail}</span>`;
  }
  ...
  function genCommand(model, c, r) {
    ...
    s += `\ntrainer = ${trainer}(model=model, args=args${isLora ? ", peft_config=peft_config" : ""}, train_dataset=ds)  # ds = load your dataset\ntrainer.train()`;
    if (c.tuning === "full" && c.numGpus > 1) {
      s += `\n\n# run (Full ${c.numGpus}-GPU): ` + (c.parallelism === "deepspeed_zero3"
  ...
  function applyDataset(id) {
    ...
    $("datasetNote").innerHTML = `<b>${esc(d.name)}</b> · ${esc(L(d.size))} · ${esc(d.format)} · ${esc(d.license)} <a class="ref-tag" href="${d.url}" target="_blank" rel="noopener">HF</a><br>` +
      `<span class="dim">${esc(L(d.why))} · ${ti("dsApplied", { o: d.objective, ex: Number(d.examples).toLocaleString(), t: d.avgTokens })}</span>`;
  ...
  function render() {
    ...
    $("modelChips").innerHTML =
      ...
      (model.n_layers ? `<span class="chip">${model.n_layers}L · h${model.hidden}</span>` : `<span class="chip">${t("archUnknown")}</span>`) +
    ...
    if (obj) $("objectiveNote").innerHTML =
      `${esc(L(obj.goal))} · <b>${esc(L(obj.dataFormat))}</b> · <a href="${obj.source}" target="_blank" rel="noopener">${esc(obj.tool)}</a>`;
    ...
    $("fitBadge").innerHTML = r.fits
      ? `<span class="badge ok">${esc(ti("fitOk", { gpu: gpu.name.split(" (")[0], n: N }))}</span>`
      : `<span class="badge no">${t("fitNo")}</span>`;
    $("vramNums").innerHTML =
      `${t("base")} <b>${fmt(r.baseGB)}GB</b> + ${t("vf_state")} <b>${fmt(r.trainStateGB)}GB</b> + ${t("vf_act")} <b>${fmt(r.activationsGB)}GB</b> + ${t("overhead")} <b>${fmt(r.overheadGB)}GB</b> ` +
      `= <b>${fmt(r.perGpuVramGB)}GB</b> / ${cap}GB${r.optimizerOffloaded ? t("pagedOptim") : ""}` +
      (r.fits ? ` · ${t("headroom")} ${fmt(r.vramHeadroomGB)}GB` : "");
    $("vramFormula").innerHTML =
      `<code>base = ${fmt(model.total_params_b,0)}B × ${r.baseBytes} byte = ${fmt(r.baseGB)}GB</code><br>` +
      `<code>${t("vf_state")} = ${fmt(r.trainableParams/1e6)}M trainable × ${r.trainPerParam} byte (grad2 + optim + master4) = ${fmt(r.trainStateGB)}GB</code><br>` +
      `<code>${t("vf_act")} ≈ batch×seq×hidden×layers×16 ${c.gradCkpt ? "× 0.22(checkpointing)" : ""} = ${fmt(r.activationsGB)}GB <span class="dim">(heuristic)</span></code><br>` +
      `<code>${r.strategy.toUpperCase()}: ${r.strategy === "fsdp" ? ti("vf_shard", { n: N }) : t("vf_ddp")}</code><br>` +
      `<span class="dim">${t("vf_src")}</span>`;
    if (!r.fits) {
      $("vramNums").innerHTML += `<div class="verdict no" style="margin-top:10px"><b>${t("oomHead")}</b><ul class="fixlist">` +
        r.oomFixes.map(f => `<li>${esc(t(f))}</li>`).join("") + `</ul></div>`;
    }

    // max model + effective batch
    $("maxModel").innerHTML = r.maxModelParamsB
      ? `<b>${fmt(r.maxModelParamsB, 0)}B</b> <span class="dim">${esc(ti("maxAt", { gpu: N + "×" + gpu.name.split(" (")[0], tuning: c.tuning.toUpperCase(), b: c.perDeviceBatch, s: c.seqLen }))}</span>`
      : "—";
    $("effBatch").innerHTML =
      `<b>${r.effectiveBatch}</b> <span class="dim">${esc(ti("effBy", { b: c.perDeviceBatch, a: c.gradAccum, n: N }))}</span>` +
      (c.gradAccum > 1 ? `<div class="dim" style="margin-top:4px">${t("accumWarn")}</div>` : "");

    // time / cost
    if (r.timeAvailable) {
      $("timeBox").innerHTML =
        `<div class="cost-row"><span>${t("trainTime")}</span><b>${fmt(r.trainHours)} h</b> <span class="dim">= ${fmt(r.trainHours/24,1)}${t("days")}</span></div>` +
        `<div class="cost-row"><span>${t("gpuHours")}</span><b>${fmt(r.gpuHours)}</b></div>` +
        (r.costUSD != null ? `<div class="cost-row"><span>${t("rentCost")} (@$${gpu.rent_usd_hr}/hr)</span><b>$${fmt(r.costUSD, 0)}</b></div>` : "") +
        `<div class="verdict" style="background:color-mix(in srgb,var(--accent) 10%, transparent)">${ti("tokAssume", { tok: fmt(r.trainTokens/1e6,1), ex: c.datasetExamples.toLocaleString(), t: c.avgTokensPerExample, ep: c.epochs, mfu: c.mfu })}</div>`;
      $("timeFormula").innerHTML =
        ...
        `<code>${t("tf_time")} = C ÷ (${gpu.flops_bf16_tf} TFLOPS × ${N} GPU × MFU ${c.mfu})${c.gradCkpt ? " × 1.3(checkpointing)" : ""} = ${fmt(r.trainHours)} h</code><br>` +
        `<span class="dim">${t("tf_src")}</span>`;
    } else {
      $("timeBox").innerHTML = `<div class="dim">${t("timeHidden")}</div>`;
      $("timeFormula").innerHTML = `<span class="dim">${t("tf_hidden")}</span>`;
    }

    // LoRA helper
    if (r.lora) {
      $("loraHelper").innerHTML =
        `<div class="cost-row"><span>${t("trainableParams")}</span><b>${fmt(r.lora.trainableParams/1e6)}M</b> <span class="dim">(${fmt(r.lora.trainablePct,2)}% of ${fmt(model.total_params_b,0)}B)</span></div>` +
        `<div class="cost-row"><span>${t("recAlpha")}</span><b>${r.lora.recommendedAlpha}</b> <span class="dim">${t("alphaRule")}</span></div>` +
        `<div class="cost-row"><span>${t("target")}</span><b>${r.lora.targetModules}</b> <span class="dim">${t("targetTip")}</span></div>` +
        `<div class="dim" style="margin-top:6px">${r.trainableSource}. ${t("lrTip")} <a href="https://magazine.sebastianraschka.com/p/practical-tips-for-finetuning-llms" target="_blank" rel="noopener">${t("basis")}</a></div>`;
    }
    ...
    if (warns.length) $("warns").innerHTML =
      `<div class="block-title">${t("warnTitle")}</div>` + warns.map(w => `<div class="verdict no" style="margin:6px 0">⚠️ ${esc(ti(w.k, w.p))}</div>`).join("");
    ...
    if (c.datasetExamples > 0) {
      ...
      $("effBatch").innerHTML += `<div class="dim" style="margin-top:4px">≈ <b>${fmt(steps, 0)}</b> ${t("steps")} ${esc(ti("stepsBy", { ex: c.datasetExamples.toLocaleString(), ep: c.epochs, eb: r.effectiveBatch }))}</div>`;
    }
  ...
  function renderReference() {
    $("refMethods").innerHTML = KB.objectives.map(o =>
      `<div class="ref-row"><span class="ref-name"><b>${esc(L(o.name))}</b><br><span class="dim">${esc(L(o.goal))}</span></span>` +
      `<span class="ref-price">${esc(L(o.dataFormat))}<br><a class="ref-tag" href="${o.source}" target="_blank" rel="noopener">docs</a></span></div>`).join("");
    $("refVolume").innerHTML = KB.dataVolume.map(v =>
      `<div class="ref-row"><span class="ref-name">${esc(L(v.goal))} <span class="ref-tag">${esc(L(v.note))}</span></span>` +
      `<span class="ref-price">${esc(L(v.range))}${v.source ? `<br><a class="ref-tag" href="${v.source}" target="_blank" rel="noopener">src</a>` : ""}</span></div>`).join("");
    $("refDistill").innerHTML = KB.distillation.map(d =>
      `<div class="ref-row"><span class="ref-name"><b>${esc(L(d.name))}</b><br><span class="dim">${esc(L(d.how))}</span></span>` +
      `<span class="ref-price">${esc(L(d.produces))}<br><a class="ref-tag" href="${d.source}" target="_blank" rel="noopener">src</a></span></div>`).join("") +
      `<div class="dim" style="margin-top:8px">${esc(L(KB._tos_warning))}</div>`;
    $("refTech").innerHTML =
      `<div class="dim" style="margin-bottom:6px">${t("coreEst")} ${TECH.established.map(e => `<a href="${e.url}" target="_blank" rel="noopener">${esc(e.name)}</a>`).join(" · ")}</div>` +
      TECH.emerging_2026.map(tc =>
        `<div class="ref-row"><span class="ref-name"><b>${esc(tc.name)}</b> <span class="ref-tag">${esc(tc.date)} · ${esc(L(tc.maturity))}</span><br><span class="dim">${esc(L(tc.what))} — ${esc(L(tc.when))}</span></span>` +
        `<span class="ref-price"><a class="ref-tag" href="${tc.url}" target="_blank" rel="noopener">src</a></span></div>`).join("");
  }

  function renderGuide() {
    if (!RECIPES) return;
    const ms = RECIPES.marketShift;
    $("marketShift").innerHTML = `<b>${esc(L(ms.headline))}</b><div class="ref-grid" style="margin-top:10px">` +
      ms.tracks.map(tr => `<div class="ref-card"><h3 style="font-size:13px">${esc(L(tr.name))}</h3><div class="dim" style="font-size:12px;line-height:1.5">${esc(L(tr.what))} ${tr.src ? `<a class="ref-tag" href="${tr.src}" target="_blank" rel="noopener">src</a>` : ""}</div></div>`).join("") + `</div>`;
    const g = RECIPES.whenToFineTune;
    $("ftGate").innerHTML = `<div class="block-title">${t("win5")}</div><ul class="fixlist">` +
      g.winConditions.map(c => `<li><b>${esc(L(c.cond))}</b> — ${esc(L(c.detail))} ${c.src ? `<a class="ref-tag" href="${c.src}" target="_blank" rel="noopener">src</a>` : ""}</li>`).join("") + `</ul>` +
      `<div class="verdict no" style="margin-top:8px">⚠️ ${t("firstRag")} ${esc(L(g.firstAskRAG))}</div>` +
      `<div class="verdict" style="margin-top:6px;background:color-mix(in srgb,var(--accent) 10%, transparent)">${t("defer")} ${esc(L(g.deferToFrontier))}</div>`;
    $("taskPicker").innerHTML = `<option value="">${t("taskPickPlaceholder")}</option>` +
      RECIPES.recipes.map(r => `<option value="${r.id}">${esc(L(r.task))}</option>`).join("");
  }
  ...
  function applyRecipe(id, setInputs) {
    ...
    const objName = L((KB.objectives.find(o => o.id === r.objective) || {}).name) || r.objective;
    const thenName = r.objectiveThen ? (L((KB.objectives.find(o => o.id === r.objectiveThen) || {}).name) || r.objectiveThen) : "";
    $("recipeDetail").innerHTML =
      `<div class="block-title">${esc(L(r.task))}</div>` +
      `<div class="cost-row"><span>${t("method")}</span><b>${esc(objName)}${thenName ? " → " + esc(thenName) : ""} · ${r.tuning.toUpperCase()} (r=${r.loraR})</b></div>` +
      `<div class="cost-row"><span>${t("data")}</span><b>${esc(L(r.dataRange))}</b></div>` +
      `<div class="cost-row"><span>${t("baseModel")}</span><b>${esc(L(r.base))}</b></div>` +
      `<div class="cost-row"><span>${t("verdict")}</span><b>${esc(L(r.verdict))}</b></div>` +
      `<div class="verdict no" style="margin-top:8px">${t("honest")} ${esc(L(r.honesty))}</div>` +
      `<div class="dim" style="margin-top:6px">${esc(L(r.note))} · ${t("recipeFilled")}</div>`;
    render();
  }
  ...
  async function boot() {
    try {
      ...
      // (re)populate selects with current-language labels; preserve selection (used on lang switch too)
      function buildSelects() {
        const keep = { dataset: $("dataset").value, model: $("model").value, gpu: $("gpu").value, objective: $("objective").value };
        ["dataset", "model", "gpu", "objective"].forEach(id => $(id).innerHTML = "");
        $("dataset").appendChild(opt("", t("dsPlaceholder")));
        DATASETS.forEach(d => $("dataset").appendChild(opt(d.id, `${d.name} · ${L(d.size)}`)));
        MODELS.forEach(m => $("model").appendChild(opt(m.id, `${m.name} · ${fmt(m.total_params_b,0)}B`)));
        GPUS.forEach(g => $("gpu").appendChild(opt(g.id, `${g.name}${g.flops_bf16_tf == null ? t("timeNA") : ""}`)));
        KB.objectives.forEach(o => $("objective").appendChild(opt(o.id, L(o.name))));
        $("dataset").value = keep.dataset;
        $("model").value = keep.model || (MODELS.find(m => m.id === "qwen3-8b") ? "qwen3-8b" : (MODELS[0] || {}).id);
        $("gpu").value = keep.gpu || "rtx4090";
        if (keep.objective) $("objective").value = keep.objective;
      }
      buildSelects();
      ...
      $("copyCmd").addEventListener("click", () => {
        const txt = $("runCmd").textContent;
        navigator.clipboard && navigator.clipboard.writeText(txt);
        $("copyCmd").textContent = t("copied"); setTimeout(() => $("copyCmd").textContent = t("copy"), 1500);
      });
      ...
      // language toggle: relabel static chrome + selects + re-render everything in the chosen language
      function switchLang(l) {
        I18N.setLang(l); I18N.applyStatic(); buildSelects();
        $("langKo").classList.toggle("active", l === "ko");
        $("langEn").classList.toggle("active", l === "en");
        renderReference(); renderGuide();
        if (lastPs) renderPresetNote(lastPs, lastOrigin);   // re-translate the preset note (keep values)
        if ($("dataset").value) applyDataset($("dataset").value);
        else if ($("taskPicker").value) applyRecipe($("taskPicker").value, true);
        else render();
      }
      $("langKo").addEventListener("click", () => switchLang("ko"));
      $("langEn").addEventListener("click", () => switchLang("en"));
      I18N.applyStatic();
      $("langKo").classList.toggle("active", I18N.getLang() === "ko");
      $("langEn").classList.toggle("active", I18N.getLang() === "en");
      renderReference();
      renderGuide();
      applyPreset(currentModel(), true); // apply preset for the initial model on load
      render();
    } catch (e) {
      $("app").innerHTML = `<div class="err">${t("loadErr")}<br><span class="dim">${esc(e.message || e)}</span></div>`;
    }
  }
  boot();
})();
